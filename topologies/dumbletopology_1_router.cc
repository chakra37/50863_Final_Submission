...
int main(int argc, char **argv) {
	...
	for(uint i = 0; i < NUM_FLOWS; ++i) {
		...
		NetDeviceContainer cright = p2pR1R2.Install(routers.Get(0), receivers.Get(i));
	...
	for (; i<NUM_FLOWS_BBR + NUM_FLOWS_CUBIC; i++){
		 prepareTCPSocket(receiverIFCs.GetAddress(i), 
											...
											START_TIME, STOP_TIME,
	 										PACKET_SIZE, 0, START_TIME+1, STOP_TIME,i); // START_TIME or DELAYED_START can be used
