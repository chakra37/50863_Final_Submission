...
#define NUM_FLOWS 8   // change this number to create different leaves on each side of dumble
#define NUM_FLOWS_BBR 1   // change this number to create different leaves on each side of dumble
#define NUM_FLOWS_CUBIC 7   // change this number to create different leaves on each side of dumble
...
#define QUEUE_SIZE       2600       // Packets
#define START_TIME       0.0       // Second
...
#define PACKET_SIZE      100000      // Bytes.
 ...
 // Create links.
  ...
  int mtu = 1500;
	...
	for (; i<NUM_FLOWS_BBR; i++){
		n3TCPSocket[i] = prepareTCPSocket(wifistaIFCs.GetAddress(i), 
											port, "ns3::TcpBbr", senders.Get(i), wifiStaNodes.Get(i), 
	...
	for (; i<NUM_FLOWS_BBR + NUM_FLOWS_CUBIC; i++){
		n3TCPSocket[i] = prepareTCPSocket(wifistaIFCs.GetAddress(i), 
											port, "ns3::TcpCubic", senders.Get(i), wifiStaNodes.Get(i), 
											START_TIME, STOP_TIME,
	...
	for (; i<NUM_FLOWS; i++){
		n3TCPSocket[i] = prepareTCPSocket(wifistaIFCs.GetAddress(i), 
											port, "ns3::TcpNewReno", senders.Get(i), wifiStaNodes.Get(i), 
											START_TIME, STOP_TIME,
	 										PACKET_SIZE, 0, START_TIME, STOP_TIME,i);
	...
	p2pNR.EnablePcapAll("wdshark", true);
